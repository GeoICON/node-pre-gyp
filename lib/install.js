...
function place_binary(uri, targetDir, opts, callback) {
  ...
  //options.agent = new HttpsProxyAgent('https://100.112.231.40:8080');

  //console.log('!!! options', options);
  //console.log('!!! AGENT4', options.agent);
  console.log('!!! URL:', sanitized);

  //requestOpts.agent = new HttpsProxyAgent('https://100.112.231.40:8080');
  requestOpts.agent = agent;

  console.log('!!! requestOpts:', requestOpts);

  fetch(sanitized, requestOpts)
    .then((res) => {
      if (!res.ok) {
        ...
        res.body.on('end', () => { console.log('!!! BODY', body) });
